...
def main(args):
    ...
    test_ds_inp = test_ds.map(lambda x: tokenizer(x['text'], max_length=256, truncation=True), batched=True).remove_columns(['text', 'label'])
    data_collator = DataCollatorWithPadding(tokenizer=tokenizer)
    data_loader = torch.utils.data.DataLoader(test_ds_inp, batch_size=128, collate_fn=data_collator, shuffle=False)
